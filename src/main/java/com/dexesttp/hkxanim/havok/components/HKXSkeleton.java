...
public class HKXSkeleton {
	...
	public static HKXSkeleton fromHKXFile(HKXFile skeletonFile, String prioritizedName) throws NoSkeletonFoundException {
		...
		for(HKXObject hkxObject : skeletonFile.getContentCollection()) {
			if(hkxObject.getDescriptor().getName().equals("hkaSkeleton")) {
				/* Not sure if this should be put in or outside of the loop. */
				result.loadDataFromSkeletonObject(hkxObject);
		...
		// Check if at least a skeleton was found.
		if(hkxSkeleton == null)
	...
	private void loadDataFromSkeletonObject(HKXObject hkxSkeleton) {
		// Display info
		System.out.println("Found skeleton : " + ((HKXStringMember) hkxSkeleton.getMembersList().get(1)).get());
		
		// Fill the Skeleton with the retrieved data.
	...
	public HKXTransform getBone(String boneName) {
